...
def acceleration(body_one, body_two, mass):

    acceleration_vector = np.array([-((G*MSUN*body_one[0])/(distance(sun_pos, body_one)**3))-(
            G*mass*(body_one[0]-body_two[0])/((distance(body_one, body_two)**3))),-(G*MSUN*body_one[1])/(
                distance(sun_pos, body_one)**3)-(G*mass*(body_one[1]-body_two[1])/(distance(body_one, body_two)**3))])
...
def earthEuler():
    ...
    for count in range(int(maxnsteps/dt)):
        ...
        earth_acceleration_vector = acceleration(earth_pos, jupiter_pos, MJUPITER)
        jupiter_acceleration_vector = acceleration(jupiter_pos, earth_pos, MEARTH)
        print(earth_acceleration_vector, jupiter_acceleration_vector)
