...
"""
# Nothing I ever do is dumb - River 2018
...
planet_position = [np.array([0.0, 0.0]), np.array(
    [1.0, 0.0]), np.array([5.2, 0.0])]
...
def distance(body_one, body_two):
    ...
    distance = math.sqrt(
        np.dot(body_one[0] - body_two[0], body_one[0] - body_two[0]))
    # print(distance)
    return distance
...
def acceleration(body_one):
    ...
    for i in range(3):
        ...
        if (distance(planet_position[i], body_one)**3) != 0:
            ...
            x_pos = planet_position[i][0]-body_one[0]
            y_pos = planet_position[i][1]-body_one[1]
            acceleration_vector[0] = acceleration_vector[0] + \
                (G*planet_mass[i]*(x_pos)) / \
                (distance(planet_position[i], body_one)**3)
            acceleration_vector[1] = acceleration_vector[1] + (G*planet_mass[i]*(
                planet_position[i][1]-body_one[1]))/(distance(planet_position[i], body_one)**3)
...
def iteration(changing_variable, update_variable):
    ...
    iterated_variable = [changing_variable[0] + update_variable[0]
                         * dt, changing_variable[1] + update_variable[1]*dt]
...
def earthEuler():
    ...
    planet_position = [np.array([0.0, 0.0]), np.array(
        [1.0, 0.0]), np.array([5.2, 0.0])]
    planet_velocity = [np.array([0.0, 0.0]), np.array(
        [0.0, 2*math.pi]), np.array([0.0, 2.68754684])]
    ...
    for _ in range(int(maxnsteps/dt)):
        ...
        earth_velocity = iteration(
            planet_position[1], earth_acceleration_vector)
        jupiter_velocity = iteration(
            planet_velocity[2], jupiter_acceleration_vector)
        # print(earth_pos)
        # print(jupiter_pos)
        ...
        e_list.append([time, earth_pos[0], earth_pos[1],
                       earth_velocity[0], earth_velocity[1]])
        j_list.append([time, jupiter_pos[0], jupiter_pos[1],
                       jupiter_velocity[0], jupiter_velocity[1]])
